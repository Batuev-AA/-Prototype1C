...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЭтаФорма.Параметры.Отбор.Свойство("ДокументХозяйственнойОперации") Тогда
		
		ДокументВладелец =  ЭтаФорма.Параметры.Отбор.ДокументХозяйственнойОперации;
		ЭтаФорма.Элементы.Декорация1.Заголовок = Строка(ДокументВладелец) ;
		
		если  ТипЗнч(ДокументВладелец) = Тип("ДокументСсылка.ПриходныйКассовыйОрдер") 
			или  ТипЗнч(ДокументВладелец) = Тип("ДокументСсылка.ПоступлениеНаРасчетныйСчет")
			тогда
			СуммаДокумента = ДокументВладелец.СуммаДокумента;
			ВЧемРазница =  "Разница в приходе: ";
			ЭтаФорма.Элементы.Декорация2.Заголовок = "Приход: " + Формат(СуммаДокумента - 0, "ЧЦ=15; ЧДЦ=2; ЧН=") + " " + ВалютаДокумента;
			
			ЭтаФорма.Элементы.СуммаРасход.Видимость = Ложь;
			
		ИначеЕсли	  ТипЗнч(ДокументВладелец) = Тип("ДокументСсылка.РасходныйКассовыйОрдер")
			или  ТипЗнч(ДокументВладелец) = Тип("ДокументСсылка.СписаниеСРасчетногоСчета")
			
			тогда
			СуммаДокумента = ДокументВладелец.СуммаДокумента;
			ВЧемРазница =  "Разница в расходе: ";
			ЭтаФорма.Элементы.Декорация2.Заголовок = "Расход: " + Формат(СуммаДокумента - 0, "ЧЦ=15; ЧДЦ=2; ЧН=") + " " + ВалютаДокумента;
			ЭтаФорма.Элементы.Ящик.Видимость = Ложь;
			ЭтаФорма.Элементы.СуммаПриход.Видимость = Ложь;
		ИначеЕсли  ТипЗнч(ДокументВладелец) = Тип("ДокументСсылка.ОперацияБух") тогда
			
			СуммаДокумента = ДокументВладелец.СуммаОперации;
			
			ЭтаФорма.Элементы.Декорация2.Заголовок = "" + Формат(ДокументВладелец.СуммаОперации, "ЧЦ=15; ЧДЦ=2") + " " + ВалютаДокумента;
		иначе
...
Процедура СчитаемИтогиНаСервере()
	...
	если  ВЧемРазница =  "Разница в приходе: " тогда
		
		ЭтаФорма.Элементы.Декорация3.Заголовок = ВЧемРазница + Формат(СуммаДокумента - ИтогиПриходПодвал, "ЧЦ=15; ЧДЦ=2; ЧН=") + " " + ВалютаДокумента;
		
	ИначеЕсли  ВЧемРазница =  "Разница в расходе: "
		тогда
		ЭтаФорма.Элементы.Декорация3.Заголовок = ВЧемРазница + Формат(СуммаДокумента - ИтогиРасходПодвал, "ЧЦ=15; ЧДЦ=2; ЧН=") + " " + ВалютаДокумента;
	иначе
	КонецЕсли;
...
Функция ПечатьАктаВскрытияЯщиковНаСервере()
	...
	Макет = УправлениеПечатью.МакетПечатнойФормы("РегистрСведений.Храм_КнигаУчета.ПФ_MXL_Храм_АктВскрытияЯщиковСбораПожертвований");
	...
	СтруктураШапки.Вставить("НомерДокумента", ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(Объект.Номер, Истина, Ложь));
...
Функция ПечатьАктаПриемаПожертвованийНаСервере()
	...
	Макет = УправлениеПечатью.МакетПечатнойФормы("РегистрСведений.Храм_КнигаУчета.ПФ_MXL_Храм_АктПриемаПожертвований");
...
Функция  ПечатьОтчетаОбОбщемПоступленииПожертвованийНаСервере()
	...
	Макет = УправлениеПечатью.МакетПечатнойФормы("РегистрСведений.Храм_КнигаУчета.ПФ_MXL_Храм_ОтчетОПоступленииПожертвований");
