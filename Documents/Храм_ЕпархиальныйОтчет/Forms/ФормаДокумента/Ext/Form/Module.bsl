...
Функция ПечатьКнигаУчетаЦелевыхНаСервере()
	...
	Макет = ДокументОбъект.ПолучитьМакет("ПФ_MXL_Храм_КнигаУчетаЦелевыхПоступлений");
	...
	Пока Выборка.СледующийПоЗначениюПоля("Ссылка") Цикл
		...
		СтруктураШапки = Новый Структура;
		СтруктураШапки.Вставить("Организация", Выборка.Организация.Наименование);
		//СтруктураШапки.Вставить("НомерДокумента", ОбщегоНазначения.НомерНаПечать(Выборка.Номер, Истина, Ложь));
		...
		СчетЦелевых = ВнешниеИсточникиДанных.БухгалтерияПредприятия30.Таблицы.ПланСчетов.НайтиПоПолю("Код", "86.02.1");
		...
		ЗапросОстатки.УстановитьПараметр("Счет", СчетЦелевых);
		ЗапросОстатки.УстановитьПараметр("Организация", Выборка.Организация);
		ЗапросОстатки.УстановитьПараметр("НачПериода", НачалоМесяца(Выборка.НачПериода));
		...
		ОстатокНаНачало = 0;
		Если  ВыборкаОстатки.Следующий() Тогда
			ОстатокНаНачало = ВыборкаОстатки.НачОстатокСумма * (-1);
			Запрос = Новый Запрос;
			Запрос.Текст = 
			"ВЫБРАТЬ
			|	Хозрасчетный.СчетДт КАК Счет,
			...
			Запрос.УстановитьПараметр("КонецПериода", Выборка.НачПериода);
			Запрос.УстановитьПараметр("НачалоПериода", ВыборкаОстатки.Период);
			...
			Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
				ОстатокНаНачало = ОстатокНаНачало - ВыборкаДетальныеЗаписи.СуммаДт + ВыборкаДетальныеЗаписи.СуммаКт; 
			КонецЦикла;
		КонецЕсли;
		...
		|	Храм_КнигаУчета.ДокументХозяйственнойОперации КАК ДокументХозяйственнойОперацииПредставление,
		...
		|	Храм_КнигаУчета.ТипДокумента КАК ТипДокумента
		|ИЗ
		|	РегистрСведений.Храм_КнигаУчета КАК Храм_КнигаУчета
		|ГДЕ
		...
		|	Храм_КнигаУчета.ТипДокумента
		|
		|УПОРЯДОЧИТЬ ПО
		|	Период";
		
		Запрос.УстановитьПараметр("НаправлениеПрихода", Справочники.Храм_НаправленияПрихода.Целевое);
		Запрос.УстановитьПараметр("СчетКассы", ВнешниеИсточникиДанных.БухгалтерияПредприятия30.Таблицы.ПланСчетов.НайтиПоПолю("Код", "50.01"));
		Запрос.УстановитьПараметр("СчетБанка", ВнешниеИсточникиДанных.БухгалтерияПредприятия30.Таблицы.ПланСчетов.НайтиПоПолю("Код", "51"));
		...
		ЗапросВИД.Текст = 
			...
			|ИЗ
			|	ВнешнийИсточникДанных.БухгалтерияПредприятия30.Таблица.Хозрасчетный КАК Хозрасчетный
			|	ГДЕ Хозрасчетный.Организация = &Организация
			|	И ДОБАВИТЬКДАТЕ(Хозрасчетный.Период, МЕСЯЦ, -24000) МЕЖДУ &НачалоПериода И КОНЕЦПЕРИОДА(&КонецПериода, ДЕНЬ)
		...
		ЗапросВИД.УстановитьПараметр("КонецПериода", Выборка.КонПериода);
		ЗапросВИД.УстановитьПараметр("НачалоПериода", Выборка.НачПериода);
		ЗапросВИД.УстановитьПараметр("Организация", Выборка.Организация);
		ЗапросВИД.УстановитьПараметр("СчетКассы", ВнешниеИсточникиДанных.БухгалтерияПредприятия30.Таблицы.ПланСчетов.НайтиПоПолю("Код", "50.01"));
		ЗапросВИД.УстановитьПараметр("СчетБанка",ВнешниеИсточникиДанных.БухгалтерияПредприятия30.Таблицы.ПланСчетов.НайтиПоПолю("Код", "51"));

		ТЗПроводки = ЗапросВИД.Выполнить().Выгрузить();

		Пока Выборка1.Следующий() Цикл
			
			ДокументСсылка = ПолучитьСсылкуНаДокумент(Выборка1.ДокументХозяйственнойОперации, Выборка1.ТипДокумента);
			
			ПараметрыОтбора = Новый Структура;
			ПараметрыОтбора.Вставить("Регистратор", ДокументСсылка.СсылкаОснование);
			СтрокаТЗ = ТЗПроводки.НайтиСтроки(ПараметрыОтбора);
			...
			СтруктураОборотСтроки.Вставить("РасшифровкаСсылка", ДокументСсылка);
			СтруктураОборотСтроки.Вставить("СчетДт", СтрокаТЗ[0].СчетДт);
			СтруктураОборотСтроки.Вставить("СчетКт", СтрокаТЗ[0].СчетКт);
		...
		ЗапросОстатки.УстановитьПараметр("Счет", СчетЦелевых);
		ЗапросОстатки.УстановитьПараметр("Организация", Выборка.Организация);
		ЗапросОстатки.УстановитьПараметр("НачПериода", НачалоМесяца(Выборка.КонПериода));
		...
		ОстатокНаКонец = 0;
		Если  ВыборкаОстатки.Следующий() Тогда
			ОстатокНаКонец = ВыборкаОстатки.НачОстатокСумма * (-1);
			Запрос = Новый Запрос;
			Запрос.Текст = 
			"ВЫБРАТЬ
			|	Хозрасчетный.СчетДт КАК Счет,
			...
			Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
				ОстатокНаКонец = ОстатокНаКонец - ВыборкаДетальныеЗаписи.СуммаДт + ВыборкаДетальныеЗаписи.СуммаКт; 
			КонецЦикла;
		КонецЕсли;
		...
		|	Хозрасчетный.СчетДт = &Счет
		|	И Хозрасчетный.Организация = &Организация
		|	И ДОБАВИТЬКДАТЕ(Хозрасчетный.Период, МЕСЯЦ, -24000) МЕЖДУ НАЧАЛОПЕРИОДА(&НачалоПериода, ДЕНЬ) И КОНЕЦПЕРИОДА(&КонецПериода, ДЕНЬ)
		|
		|СГРУППИРОВАТЬ ПО
		|	Хозрасчетный.СчетДт
		|
		...
		Приход86 = 0;
		Пока  ВыборкаДетальныеЗаписи.Следующий() Цикл
			Расход86 =  Расход86 + ВыборкаДетальныеЗаписи.СуммаДТ;
			Приход86 =  Приход86 + ВыборкаДетальныеЗаписи.СуммаКТ;
		КонецЦикла;
...
Процедура ПечатьКнигаУчетаЦелевых(Команда)
	...
	ПараметрыОткрытия.Вставить("ПечатнаяФорма", ТабДокумент);
	ФормаТабДок = ПолучитьФорму("Документ.Храм_ЕпархиальныйОтчет.Форма.ФормаПечати",ПараметрыОткрытия);
	ФормаТабДок.Открыть(); 	
КонецПроцедуры
...
Функция ПечатьКнигаУчетаОбщихНаСервере()
	...
	ДокументОбъект = РеквизитФормыВЗначение("Объект");
	Макет = ДокументОбъект.ПолучитьМакет("ПФ_MXL_Храм_КнигаУчетаОбщихПоступлений");
	...
	Пока Выборка.СледующийПоЗначениюПоля("Ссылка") Цикл
		...
		СчетОбщих = ВнешниеИсточникиДанных.БухгалтерияПредприятия30.Таблицы.ПланСчетов.НайтиПоПолю("Код", "86.01");
		...
		|	ВнешнийИсточникДанных.БухгалтерияПредприятия30.Таблица.Хозрасчетный_ИтогиПоСчетам КАК Хозрасчетный_ИтогиПоСчетам
		|ГДЕ
		|	Хозрасчетный_ИтогиПоСчетам.Счет.Код ПОДОБНО &Счет
		|	И Хозрасчетный_ИтогиПоСчетам.Организация = &Организация
		|	И ДОБАВИТЬКДАТЕ(Хозрасчетный_ИтогиПоСчетам.Период, МЕСЯЦ, -24000) <= &НачПериода
		|
		|УПОРЯДОЧИТЬ ПО
		...
		ЗапросОстатки.УстановитьПараметр("Счет", "86.01%");
		ЗапросОстатки.УстановитьПараметр("Организация", Выборка.Организация);
		ЗапросОстатки.УстановитьПараметр("НачПериода", НачалоМесяца(Выборка.НачПериода));
		...
		Если  ВыборкаОстатки.Следующий() Тогда
			...
			|	ВнешнийИсточникДанных.БухгалтерияПредприятия30.Таблица.Хозрасчетный КАК Хозрасчетный
			|ГДЕ
			|	Хозрасчетный.СчетДт.Код ПОДОБНО &Счет
			|	И Хозрасчетный.Организация = &Организация
			|	И ДОБАВИТЬКДАТЕ(Хозрасчетный.Период, МЕСЯЦ, -24000) МЕЖДУ &НачалоПериода И НАЧАЛОПЕРИОДА(&КонецПериода, ДЕНЬ)
			|
			|СГРУППИРОВАТЬ ПО
			...
			|	ВнешнийИсточникДанных.БухгалтерияПредприятия30.Таблица.Хозрасчетный КАК Хозрасчетный
			|ГДЕ
			|	Хозрасчетный.СчетКт.Код ПОДОБНО &Счет
			|	И Хозрасчетный.Организация = &Организация
			|	И ДОБАВИТЬКДАТЕ(Хозрасчетный.Период, МЕСЯЦ, -24000) МЕЖДУ &НачалоПериода И НАЧАЛОПЕРИОДА(&КонецПериода, ДЕНЬ)
			|
			|СГРУППИРОВАТЬ ПО
			...
			Запрос.УстановитьПараметр("НачалоПериода", ВыборкаОстатки.Период);
			Запрос.УстановитьПараметр("Организация", Выборка.Организация);
			Запрос.УстановитьПараметр("Счет", "86.01%");
		...
		Пока Выборка1.Следующий() Цикл
			...
			ОборотСтрокиТаблицы.Параметры.Заполнить(СтруктураОборотСтроки); 
			ОборотСтрокиТаблицы.Параметры.Ссылка = Выборка.Ссылка;
...
Функция ПечатьКнигаУчетаНеразнесенныхНаСервере()
	...
	ДокументОбъект = РеквизитФормыВЗначение("Объект");
	Макет = ДокументОбъект.ПолучитьМакет("ПФ_MXL_Храм_КнигаУчетаНеразнесенныхОпераций");
	...
	Пока Выборка.СледующийПоЗначениюПоля("Ссылка") Цикл
		...
		ЗапросКнигаУчета = Новый Запрос;               //Запрос такой хитрый потому что к одному документу надо выбрать одну проводку
		ЗапросКнигаУчета.Текст = "ВЫБРАТЬ
		|	Храм_КнигаУчета.Период КАК Период,
		|	Храм_КнигаУчета.ДокументХозяйственнойОперации КАК ДокументХозяйственнойОперации
		|ИЗ
		|	РегистрСведений.Храм_КнигаУчета КАК Храм_КнигаУчета
		...
		ЗапросКнигаУчета.УстановитьПараметр("НачПериода", Выборка.НачПериода);
		ЗапросКнигаУчета.УстановитьПараметр("КонПериода", Выборка.КонПериода);
		
		ТаблицаКнигиУчета = ЗапросКнигаУчета.Выполнить().Выгрузить();
		
		ЗапросВИД = Новый Запрос;               //Запрос такой хитрый потому что к одному документу надо выбрать одну проводку
		ЗапросВИД.Текст =  "ВЫБРАТЬ
           |	ДОБАВИТЬКДАТЕ(Хозрасчетный.Период, МЕСЯЦ, -24000) КАК Период,
           |	Хозрасчетный.Регистратор КАК ДокументХозяйственнойОперации,
           |	ВЫБОР
           ...
           |	КОНЕЦ КАК СуммаРасход,
			...
			|	ВнешнийИсточникДанных.БухгалтерияПредприятия30.Таблица.Хозрасчетный КАК Хозрасчетный
			|ГДЕ
			|	ДОБАВИТЬКДАТЕ(Хозрасчетный.Период, МЕСЯЦ, -24000) МЕЖДУ НАЧАЛОПЕРИОДА(&НачПериода, ДЕНЬ) И КОНЕЦПЕРИОДА(&КонПериода, ДЕНЬ)
	        |	И (Хозрасчетный.СчетДт = &СчетКассы
	        |			ИЛИ Хозрасчетный.СчетДт = &СчетБанка
	        |			ИЛИ Хозрасчетный.СчетКт = &СчетКассы
	        |			ИЛИ Хозрасчетный.СчетКт = &СчетБанка)
		...
		ЗапросВИД.УстановитьПараметр("НаправлениеПрихода", Справочники.Храм_НаправленияПрихода.Нецелевое);
		ЗапросВИД.УстановитьПараметр("СчетКассы", ВнешниеИсточникиДанных.БухгалтерияПредприятия30.Таблицы.ПланСчетов.НайтиПоПолю("Код", "50.01"));
		ЗапросВИД.УстановитьПараметр("СчетБанка", ВнешниеИсточникиДанных.БухгалтерияПредприятия30.Таблицы.ПланСчетов.НайтиПоПолю("Код", "51"));
		ЗапросВИД.УстановитьПараметр("НачПериода", Выборка.НачПериода);
		ЗапросВИД.УстановитьПараметр("КонПериода", Выборка.КонПериода);
		 
		Результат1 = ЗапросВИД.Выполнить();
		Выборка1 = Результат1.Выбрать();
		СтруктураОборотСтроки = Новый Структура;
		
		ТаблицаВнешнегоИсточника = ЗапросВИД.Выполнить().Выгрузить();
		Остаток = ОстатокНаНачало;
		ОборотПриход = 0;
		ОборотРасход = 0;
		...
		Пока Выборка1.Следующий() Цикл
			...
			ДокументСсылка = ВнешниеИсточникиДанных.БухгалтерияПредприятия30.Таблицы.ПоступлениеНаРасчетныйСчет.НайтиПоПолю("СсылкаОснование",Выборка1.ДокументХозяйственнойОперации);
			Если Не ЗначениеЗаполнено(ДокументСсылка) тогда
				ДокументСсылка = ВнешниеИсточникиДанных.БухгалтерияПредприятия30.Таблицы.ПриходныйКассовыйОрдер.НайтиПоПолю("СсылкаОснование",Выборка1.ДокументХозяйственнойОперации);
			КонецЕсли;
			Если Не ЗначениеЗаполнено(ДокументСсылка) тогда
				ДокументСсылка = ВнешниеИсточникиДанных.БухгалтерияПредприятия30.Таблицы.РасходныйКассовыйОрдер.НайтиПоПолю("СсылкаОснование",Выборка1.ДокументХозяйственнойОперации);
			КонецЕсли;
			Если Не ЗначениеЗаполнено(ДокументСсылка) тогда
				ДокументСсылка = ВнешниеИсточникиДанных.БухгалтерияПредприятия30.Таблицы.СписаниеСРасчетногоСчета.НайтиПоПолю("СсылкаОснование",Выборка1.ДокументХозяйственнойОперации);
			КонецЕсли;
			
			ПараметрыОтбора = Новый Структура;
		    ПараметрыОтбора.Вставить("ДокументХозяйственнойОперации", Строка(ДокументСсылка));
			Если ТаблицаКнигиУчета.НайтиСтроки(ПараметрыОтбора).Количество() <> 0 тогда
				Продолжить;
			КонецЕсли;
			
			СтруктураОборотСтроки.Вставить("Дата", Формат(Выборка1.Период,"ДЛФ=Д"));
			СтруктураОборотСтроки.Вставить("Содержание", "" +  Строка(ДокументСсылка));
			СтруктураОборотСтроки.Вставить("РасшифровкаСсылка", ДокументСсылка);
...
Функция ПолучитьСсылкуНаДокумент(Представление, ТипДокумента)
	ДокументСсылка = Неопределено;
	Если ТипДокумента = "ПриходныйКассовыйОрдер" тогда
		НомерДокумента =  Сред(Представление, 23, 11);
		ДокументСсылка = ВнешниеИсточникиДанных.БухгалтерияПредприятия30.Таблицы.ПриходныйКассовыйОрдер.НайтиПоПолю("Номер",НомерДокумента);
	ИначеЕсли ТипДокумента = "РасходныйКассовыйОрдер" тогда
		НомерДокумента =  Сред(Представление, 18, 11);
		ДокументСсылка = ВнешниеИсточникиДанных.БухгалтерияПредприятия30.Таблицы.РасходныйКассовыйОрдер.НайтиПоПолю("Номер",НомерДокумента);
	ИначеЕсли ТипДокумента = "ПоступлениеНаРасчетныйСчет" тогда
		НомерДокумента =  Сред(Представление, 32, 11);
		ДокументСсылка = ВнешниеИсточникиДанных.БухгалтерияПредприятия30.Таблицы.ПоступлениеНаРасчетныйСчет.НайтиПоПолю("Номер",НомерДокумента);
	ИначеЕсли ТипДокумента = "СписаниеСРасчетногоСчета" тогда
		НомерДокумента =  Сред(Представление, 28, 11);
		ДокументСсылка = ВнешниеИсточникиДанных.БухгалтерияПредприятия30.Таблицы.СписаниеСРасчетногоСчета.НайтиПоПолю("Номер",НомерДокумента);
	КонецЕсли;
