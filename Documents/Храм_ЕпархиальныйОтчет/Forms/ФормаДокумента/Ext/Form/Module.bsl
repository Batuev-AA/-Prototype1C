...
//&НаСервере
//Процедура ОрганизацияПриИзмененииНаСервере(Валюта)
//	
//	
...
//		ОрганизацияПриИзмененииНаСервере(ВалютаРегламентированногоУчета);
//	КонецЕсли;
//	
//КонецПроцедуры
...
//	НовыйМассивПараметров.Добавить(Новый ПараметрВыбора("Отбор.Владелец", Форма.ВладелецСчетаОрганизации));
...
Функция ПечатьЕпархиальныйОтчет() Экспорт
	...
	Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.Храм_ЕпархиальныйОтчет.ПФ_MXL_Храм_ЕпархиальныйОтчет");
	...
	Пока Выборка.СледующийПоЗначениюПоля("Ссылка") Цикл
		...
		СтруктураШапки.Вставить("Организация",   ОбщегоНазначенияБПВызовСервера.ОписаниеОрганизации(СведенияОбОрганизации, "НаименованиеДляПечатныхФорм, Телефоны"));
		...
		РасчетныйСчет =  ОбщегоНазначенияБПВызовСервера.ОписаниеОрганизации(СведенияОбОрганизации, "НомерСчета");
		Для сч = 1 по 20 Цикл
			СтруктураШапки.Вставить("Р"+сч,  Сред(РасчетныйСчет,сч+4,1));
		КонецЦикла;
		
		ИНН =  ОбщегоНазначенияБПВызовСервера.ОписаниеОрганизации(СведенияОбОрганизации, "ИНН");
		Для сч = 1 по 10 Цикл
			СтруктураШапки.Вставить("И"+сч,  Сред(ИНН,сч+4,1));
		КонецЦикла;
		
		БИК =  ОбщегоНазначенияБПВызовСервера.ОписаниеОрганизации(СведенияОбОрганизации, "БИК");
		Для сч = 1 по 9 Цикл
			СтруктураШапки.Вставить("Б"+сч,  Сред(БИК,сч+4,1));
		КонецЦикла;
		
		КоррСчет =  ОбщегоНазначенияБПВызовСервера.ОписаниеОрганизации(СведенияОбОрганизации, "КоррСчет");
		Для сч = 1 по 20 Цикл
...
Процедура ЗаполнитьПоДаннымБухучетаНаСервере()
	 ...
	 СчетКассы = ПланыСчетов.Хозрасчетный.КассаОрганизации;
	...
	Если  Выборка.Следующий() Тогда
		...
		Объект.ОстатокСредствНаНачалоПериодаВКассе = Выборка.СуммаОстаток;
	...
	Если  Выборка.Следующий() Тогда
		...
		Объект.ОстатокСредствНаНачалоПериодаВБанке = Выборка.СуммаОстаток;
	...
	Объект.ОстатокСредствНаНачалоПериодаИтого =  Объект.ОстатокСредствНаНачалоПериодаВКассе + Объект.ОстатокСредствНаНачалоПериодаВБанке;
	...
	Если  Выборка.Следующий() Тогда
		...
		Объект.ОстатокСредствНаДатуОтчетаВКассе = Выборка.СуммаОстаток;
		
	иначе
		
		Объект.ОстатокСредствНаДатуОтчетаВКассе = 0;
	...
	Если  Выборка.Следующий() Тогда
		...
		Объект.ОстатокСредствНаДатуОтчетаВБанке = Выборка.СуммаОстаток;
	...
	Объект.ОстатокСредствНаДатуОтчетаИтого =  Объект.ОстатокСредствНаДатуОтчетаВКассе + Объект.ОстатокСредствНаДатуОтчетаВБанке;
...
МассивОбъектов = Новый Массив;
	...
	Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.Храм_ЕпархиальныйОтчет.ПФ_MXL_Храм_КнигаУчетаЦелевыхПоступлений");
...
МассивОбъектов = Новый Массив;
	...
	Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.Храм_ЕпархиальныйОтчет.ПФ_MXL_Храм_КнигаУчетаОбщихПоступлений");
...
МассивОбъектов = Новый Массив;
	...
	Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.Храм_ЕпархиальныйОтчет.ПФ_MXL_Храм_КнигаУчетаНеразнесенныхОпераций");
